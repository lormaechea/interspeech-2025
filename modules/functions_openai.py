...
def process_simplifications(gen_client, eval_client, sentence_id, sentence, prompt_id, prompt, history, gen_model, eval_model):
	...
	for iteration in range(1, 6):
		...
		try:
			if iteration == 1:
				simplification, history = run_gen_openai(gen_client, sentence, history, gen_model)
				...
				simplification, history = run_feedback_openai(gen_client, sentence, simplification, feedback, history, gen_model)

			print(f"Simplification: {simplification}")
			
			scores = run_eval_openai(eval_client, sentence, simplification, eval_model)
			...
			if None in [scores["simp"]["score"], scores["mean"]["score"], scores["gram"]["score"]]:
				...
				final_score = run_topeval_openai(eval_client, sentence, simplification, eval_model, 
...
def run_feedback_openai(client, sentence, simplification, feedback, history, model):
...
def run_gen_openai(client, sentence, history, model):
	user_prompt = """
...
"""
	...
	response = client.chat.completions.create(
		model=model,  
		messages=history, # Pass the conversation history, including all system/user/assistant messages
		temperature=params["gen"]["temperature"],
		max_tokens=params["gen"]["max_new_tokens"], 
		top_p=params["gen"]["top_p"],  
...
def run_eval_openai(client, sentence, simplification, model):
	...
	mean_response = client.chat.completions.create(
		model=model,
		messages=[
			{"role": "user", "content": prompt}
		],
	...
	gram_response = client.chat.completions.create(
		model=model,
		messages=[
			{"role": "user", "content": prompt}
		],
...
def run_topeval_openai(client, sentence, simplification, model, s, m, g, s_score, m_score, g_score):
